...
class A2Topology( Topo ):
    ...
    def build( self ):
        ...
        # Set BW
        self.addLink( s1, s2, bw=4 ) # high bw = 2-4Mbps per user (video + voice)
        self.addLink( s1, s3, bw=0.1,delay='5ms',jitter='5ms',loss=2) # poor bw = 100kbps (min) per user (voice)
        self.addLink( s1, s4, bw=1,delay='0.5ms',jitter='2ms',loss=0.5) # low bw = 1Mbps per user (datanetwork)
